-- Follow-up to 20260913150000: the idempotency guarantee relied on a
-- SELECT-then-INSERT check, which is safe against a double-click or a
-- retry that happens after the first call has already committed, but
-- is NOT safe against two calls whose idempotency-check SELECTs both run
-- before either has committed (a genuine simultaneous-transaction race,
-- e.g. two browser tabs, or a network-level automatic retry that fires
-- while the original request is still in flight). In that exact window,
-- both calls would pass the SELECT check and both would attempt the
-- INSERT — the UNIQUE index on registration_idempotency_key stops a
-- second row from ever being created, but without this fix the loser
-- would surface a raw "duplicate key" Postgres error to the caller
-- instead of the same successful result the winner got.
--
-- Wraps just the member INSERT in its own sub-transaction (a plpgsql
-- BEGIN/EXCEPTION block, which Postgres implements as a savepoint) so a
-- unique_violation specifically on the idempotency key falls through to
-- returning the winner's result, while any other unique violation (e.g.
-- a genuine barcode collision) still surfaces as a real error.
...
BEGIN
  ...
  -- Nested block (implemented as a savepoint): a unique_violation here can
  -- only mean either (a) another concurrent call with the SAME
  -- idempotency_key won the race, in which case we return its result
  -- instead of erroring, or (b) an unrelated collision (membership_no
  -- computed under the same advisory lock should never collide;
  -- barcode is the one other UNIQUE column, and that's always NULL
  -- here), which re-raises as a real error.
  BEGIN
